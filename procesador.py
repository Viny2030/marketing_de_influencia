...
def anonimizar_email(email):
    """Aplica SHA-256 para cumplir con GDPR/LGPD."""
...
def calcular_atribucion_probabilistica(df_ventas, df_vistas, ventana_horas=24):
    ...
    por un video basándose en la cercanía temporal.
    """
    atribuciones = []
    
    for _, venta in df_ventas.iterrows():
        # Buscamos impactos en la ventana de tiempo previa a la venta
        ...
        if not impactos.empty:
            # Cuanto más cerca está el posteo de la venta, mayor es el score
            impactos = impactos.copy()
            impactos['diff_horas'] = (venta['timestamp'] - impactos['timestamp']).dt.total_seconds() / 3600
            impactos['score_influencia'] = np.exp(-impactos['diff_horas'] / 12) # Decaimiento exponencial
